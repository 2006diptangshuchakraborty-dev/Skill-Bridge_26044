...
import { user, session, account, verification } from "@/db/schema/user.js";

import { resolveValidIntent, markIntentUsed } from "@/lib/signup-intent";
import { resolveAppBaseUrl } from "@/lib/app-url.js";
...
const betterAuthURL = resolveAppBaseUrl();
    ...
    user: {
      create: {
        ...
        before: async (newUser, context) => {
          ...
          if (request) {
            try {
              const requestURL = new URL(request.url || resolveAppBaseUrl());
              ...
              if (!intentToken && request.headers) {
                ...
                if (cookieHeader) {
                  ...
                  if (match) {
                    intentToken = decodeURIComponent(match[1]);
                  }
                }
              }
            } catch (error) {
          ...
          if (intentToken) {
            try {
              const validIntent = await resolveValidIntent(intentToken);
